    ...
    return (
        <div className="space-y-12 pb-20">
            ...
            <section className="space-y-6 mt-12">
                ...
                <div className="glass-card p-2 rounded-[40px] border-white/5">
                    <div className="space-y-2">
                        {topUsers.map((user: any, index: number) => (
                            <div key={user.id} className="flex items-center justify-between p-6 hover:bg-white/5 rounded-[32px] transition-all group">
                                <div className="flex items-center gap-6">
                                    <div className="w-12 h-12 rounded-2xl bg-muted/50 flex items-center justify-center font-black text-lg text-muted-foreground group-hover:text-primary transition-colors">
                                        #{index + 1}
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="font-black text-lg uppercase tracking-tight group-hover:translate-x-1 transition-transform">
                                            {user.name || `User ${user.id.slice(0, 5)}...`}
                                        </span>
                                        <span className="text-[10px] text-muted-foreground font-bold tracking-widest uppercase opacity-50">
                                            Member since {new Date(user.createdAt).getFullYear()}
                                        </span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-black text-muted-foreground uppercase mb-1 tracking-widest">Total Wealth</p>
