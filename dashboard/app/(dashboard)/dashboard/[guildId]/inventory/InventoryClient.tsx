...
export const InventoryClient = ({ initialSkins, userId }: InventoryClientProps) => {
    ...
    const handleSellSelected = async () => {
        ...
        if (res.success) {
            ...
            alert(res.error || "Gagal jual skin bang.");
        }
        setIsSelling(false);
    };
    ...
    return (
        <div className="space-y-6">
            {/* Controls Bar */}
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between p-6 glass-card rounded-3xl border-white/5">
                <div className="flex flex-1 gap-4 w-full">
                    ...
                    <div className="relative group">
                        <select
                            ...
                            className="bg-white/5 border border-white/10 rounded-2xl py-3 px-10 appearance-none focus:outline-none focus:border-primary text-sm font-bold transition-all cursor-pointer min-w-[140px]"
                        >
                    ...
                    <div className="relative group">
                        <select
                            ...
                            className="bg-white/5 border border-white/10 rounded-2xl py-3 px-10 appearance-none focus:outline-none focus:border-primary text-sm font-bold transition-all cursor-pointer min-w-[160px]"
                        >
                            <option value="price-desc" className="bg-[#0f1115]">Harga: Mahal</option>
                            <option value="price-asc" className="bg-[#0f1115]">Harga: Murah</option>
                            <option value="rarity" className="bg-[#0f1115]">Rarity: Tinggi</option>
                        </select>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-white/5">
                    <button
                        ...
                        className="px-6 py-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-black uppercase transition-all whitespace-nowrap"
                    >
                        {selectedIds.length === filteredSkins.length ? "Batal Semua" : "Pilih Semua"}
                    </button>

                    <button
                        onClick={handleSellSelected}
                        disabled={selectedIds.length === 0 || isSelling}
                        className="flex-1 md:flex-none px-8 py-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-black uppercase tracking-widest hover:bg-emerald-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-30"
                    >
