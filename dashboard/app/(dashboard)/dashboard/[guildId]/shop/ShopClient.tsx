...
export default function ShopClient({ userWallet }: ShopClientProps) {
    ...
    const handleBuy = async (item: GameItem) => {
        if (userWallet < item.price) {
            alert("Duit lu kaga cukup bang! Kerja dulu gih.");
            return;
        }
        ...
        if (res.success) {
            // Success feedback would be nice, but confirm/alert is enough for now
            // Maybe a toast in the future
            router.refresh();
        } else {
            alert(res.error || "Gagal beli item. Coba lagi deh.");
        }
        setIsBuying(null);
    };
