...
export const getWeightedSkin = (skins: any[], caseType: CaseType = "highroller", pityCount: number = 0, customWeights?: any, streak: number = 0) => {
    ...
    // Apply Streak Multiplier to High Tiers (Boost only for real wins)
    ...
    // Ensure weights don't drop to 0 for lower tiers if we want to keep them somewhat consistent,
    // actually, lowering high tier weights naturally increases probability of low tiers relative to total.
    // So we just modify high tiers.
    ...
    if (possibleSkins.length === 0) {
        // Fallback safety: If target rarity fetch fails, try to fetch the lowest rarity allowed in this crate
        // instead of returning a completely random skin (which might be Consumer grade sampah).
        const lowestRarity = Object.keys(weights)[0]; // Weights are usually ordered low to high
        ...
        // Only if THAT fails too, return global random
...
export const getSkinPrice = async (skin: any, float: number): Promise<number> => {
    ...
    if (rarityName.includes("industrial")) basePrice = 3000 + Math.random() * 8000; // 3k - 11k
    else if (rarityName.includes("mil-spec")) basePrice = 20000 + Math.random() * 40000; // 20k - 60k (Safe drop for Classic)
    else if (rarityName.includes("restricted")) basePrice = 75000 + Math.random() * 175000; // 75k - 250k (Safe-ish for Highroller)
    else if (rarityName.includes("classified")) basePrice = 350000 + Math.random() * 750000; // 350k - 1.1M
    else if (rarityName.includes("covert")) basePrice = 2000000 + Math.random() * 5000000; // 2M - 7M
    else if (rarityName.includes("contraband") || rarityName.includes("extraordinary") || rarityName.includes("gold")) {
...
export const getSkinPriceSync = (rarityName: string, float: number): number => {
    ...
    if (r.includes("industrial")) basePrice = 3000 + Math.random() * 8000; // 3k - 11k
        ...
        basePrice = 15000000 + Math.random() * 35000000;
    } else {
